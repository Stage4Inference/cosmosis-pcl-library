...
def get_ell_scales_nmt(workspace):

    nell_eff = workspace.get_bandpower_windows().shape[1]

    b = workspace.wsp.bin

    ell_mids = nmt.nmtlib.get_ell_eff(b, nell_eff)

    nell_list = nmt.nmtlib.get_nell_list(b, nell_eff)

    ell_lims = np.zeros(nell_eff+1)
    for i in range(nell_eff):
        ell_lims[i] = nmt.nmtlib.get_ell_list(b, i, int(nell_list[i]))[0]
    ell_lims[-1] = nmt.nmtlib.get_ell_list(b, nell_eff-1, int(nell_list[-1]))[-1]
    ell_lims = [int(x) for x in ell_lims]
    return ell_lims, ell_mids
...
def setup(options):
    ...
    if options.has_value(option_section, "theta_min"):
        ...
        nmt_ell = options.get_bool(option_section, "nmt_ell", False)
        if nmt_ell :
            workspace_name = options.get_string(option_section, "nmt_workspace", "")
            workspace = nmt.NmtWorkspace()
            workspace.read_from(workspace_name)
        ...
        if nmt_ell:
            ell_lims, ell_mids = get_ell_scales_nmt(workspace)
        else:
...
def execute(block, config):
    ...
    if make_covariance:
        ...
        if real_space:
            ...
            print(sum([len(s.value) for s in spec_meas_list]), covmat.shape[0])
