...
from projection_tools.fastpt_tools import get_bias_params_redshift,\
    get_PXX_red_dep, get_PXm_bias_red_dep
...
class NlgalredNlgalredSpectrum(LingalLingalSpectrum):
    """
    Class for computing galaxy C(l) with non-linear bias models.
    ...
    def load_bias(self, block, sample, pt_type):
        bias_values = get_bias_params_redshift(block, pt_type, sample)
        nbin = self.source.kernels[sample].nbin
        divout_lin_bias_values = {}
        for bin_num in range(1, nbin+1):
            divout_lin_bias_values[bin_num] = bias_values["b1E"][0]
        return bias_values, divout_lin_bias_values
    ...
    def set_power(self, block, bin1, bin2):
        ...
        # Get the full non-linear P_gg(k) from fastpt
        P_gg, P_gg_terms = get_PXX_red_dep(self.bias_values_a,
            self.bias_values_b, self.Pk_basis_funcs,
            self.pt_type)
    ...
    def get_power_spline(self, block, bin1, bin2):
        ...
        #TODO : I could do call set_power only once as Pgg(z,k) is the same
        # for all bins.
        # A way of doing this would be to call set_power only if
        # self.current_bini are None.
        if self.current_bin1 != bin1 or self.current_bin2 != bin2:
            self.set_power(block, bin1, bin2)
        return self.pk_chi_logk_spline
...
class NlgalredShearSpectrum(LingalLensingSpectrum):
    """
    Class for computing galaxy C(l) with non-linear bias models. For this case
    the linear power spectrum used in the exact calculation is still
    just the linear matter power spectrum (and the b_1 values are passed
    ...
    def set_power(self, block, bin1, bin2):
        ...
        # Get the full non-linear P_gg(k) from fastpt
        P_gm, P_gm_terms = get_PXm_bias_red_dep(self.bias_values_a,
            self.Pk_basis_funcs, self.pt_type)
...
class SpectrumType(Enum):
    ...
    class NlgalredNlgalred(NlgalredNlgalredSpectrum):
        power_3d_type = MatterPower3D
        kernel_types = ("N", "N")
        autocorrelation = True
        name = "galaxy_cl"
    ...
    class NlgalredShear(NlgalredShearSpectrum):
        autocorrelation = False
        power_3d_type = MatterPower3D
        kernel_types = ("N", "W")
        autocorrelation = False
...
class SpectrumCalculator(object):
    ...
    def __init__(self, options):
        ...
        # Convolution
        self.mask_convolution = options.get_string(option_section, "mask_convolution", None)
        self.NmtWorkspace_name = options.get_string(option_section, "NmtWorkspace_name", "")
        ...
        # Load the NaMaster workspaces for the theory convolution
        if self.mask_convolution != 'None':
            self.workspace_dic = {}
            for spectrum in self.req_spectra :
                self.workspace_dic[spectrum.section_name] = nmt.NmtWorkspace()
                self.workspace_dic[spectrum.section_name].read_from\
                    (f'{self.NmtWorkspace_name}_{spectrum.section_name}.fits')
    ...
    def bin_cell(self, cls_in, workspace):
        """ Bins a power spectrum into bandpowers. This is carried
        out as a weighted average over the multipoles in each bandpower. Taken
        from the Nmt.Bin module.
        ...
        if (cls_in.ndim > 2) or (len(cls_in[0]) != wsp.lmax + 1):
            raise ValueError("Input Cl has wrong size")
        cl1d = nmt.nmtlib.bin_cl(bin, cls_in, len(cls_in) * bin.n_bands)
        clout = np.reshape(cl1d, [len(cls_in), bin.n_bands])
        if oned:
            clout = clout[0]
        return clout

    # Convolve theory Cl with the mask. /!\ ONLY FOR SPIN 0 FIELDS
    # The convolution need the Cl at all ell from 0 to lmax, where lmax is the maximum ell used in the
    # given binning scheme, which is inside the NaMaster workspace. As doing the projection at all ell
    # would be very long we interpolate the computed Cl on this large ell grid to then perform the convolution.
    def convolve(self, ell, cl, spectrum_name):
        workspace = self.workspace_dic[spectrum_name]
        lmax_wsp = workspace.wsp.lmax # Get the maximum ell used in the given binning scheme
        ell_all = np.arange(0,lmax_wsp+1) # Get all ell between 0 and lmax
        interp_cl = interp.interp1d(ell, cl, fill_value="extrapolate", bounds_error=False)(ell_all) # Interpolate the Cl

        if spectrum_name == 'galaxy_cl':
            cl_in = np.array([interp_cl])

        elif spectrum_name == 'galaxy_shear_cl':
            cl_in = np.array([interp_cl, np.zeros_like(interp_cl)])

        elif spectrum_name == 'shear_cl':
            cl_in = np.array([interp_cl, np.zeros_like(interp_cl),
                              np.zeros_like(interp_cl), np.zeros_like(interp_cl)])

        else:
            raise ValueError(f"This spectrum type '{self.spectrum_name}'"
                             "Is not yet supported by the convolution")

        if self.mask_convolution == 'coupled':
            conv_cl = workspace.couple_cell(np.array(cl_in))
            conv_cl = self.bin_cell(conv_cl, workspace)

        elif self.mask_convolution == 'decoupled':
            conv_cl = workspace.decouple_cell(workspace.couple_cell(cl_in)) # Convolve the Cl in the NaMaster way

        else:
            raise ValueError(f"Invalid convolution type: {self.mask_convolution}."
                             "Should be None or 'coupled' or 'decoupled'")
    ...
    def compute_spectrum(self, block, spectrum):
        ...
        for i in range(na):
            ...
            for j in range(jmax):
                ...
                # Convolution with the mask if asked
                if self.mask_convolution != 'None':
                    c_ell_tosave = self.convolve(ell, c_ell, spectrum.section_name)
                    binning = list(self.workspace_dic.items())[0][1].wsp.bin
                    ell_tosave = nmt.nmtlib.get_ell_eff(binning,
                                                        c_ell_tosave.size)
                else :
                    c_ell_tosave = c_ell
                    ell_tosave = ell

                block[spectrum.section_name, sep_name] = ell_tosave
                block[spectrum.section_name, f'bin_{i+1}_{j+1}'] = c_ell_tosave
