...
def get_bias_params_redshift(block, pt_type, sample,
                             matter_power_nl_name = names.matter_power_nl):
    """
    Load bias values for a given bin from the block.
    ...
    """
    bias_section = f"bias_{sample}"
    z_pk, _, _ = block.get_grid(matter_power_nl_name, "z", "k_h", "p_k")

    #b1E is always required
    b1E_poly = [block[bias_section, f"b1E_poly{i}"] for i in range(4)]

    if pt_type == 'oneloop_eul_bk':
        bias_order = ['b1E', 'b2E', 'bsE', 'b3nlE', 'bkE']
        param_names = [[f"b1E_poly{i}" for i in range(4)],
                       [f"b2E_poly{i}" for i in range(4)],
                       [f"bsE_poly{i}" for i in range(4)],
                       [f"b3nlE_poly{i}" for i in range(4)],
                       [f"bkE_poly{i}" for i in range(4)]]
        param_defaults = [[None for i in range(4)],
                          [0.0 for i in range(4)],
                          [(-4. / 7.) * (b1E_poly[0] - 1)] + [(-4. / 7.) * b1E_poly[i] for i in range(1,4)],
                          [(b1E_poly[0] - 1)] + [b1E_poly[i] for i in range(1,4)],
                          [0.0 for i in range(4)]]
    ...
    bias_values = {}
    #Loop through bias param names and read in to dictionary
    for param_name, default, order in zip(param_names, param_defaults, bias_order):
        bias_coeff = [block.get_double(bias_section,
            param_name[i], default[i]) for i in range(4)]
        bias_values[order] = np.poly1d(list(reversed(bias_coeff)))(z_pk)
...
def get_PXX_red_dep(bias_values_bin1, bias_values_bin2, Pk_basis_funcs, pt_type):
    """
    Get P_XX(k) - the tracer-tracer power spectrum, generated by summing
    the P(k) terms in Pk_basis_funcs with the bias coefficients in
    bias_values_bin1 and bias_values_bin2. Also return the individual
    ...
    if pt_type == "oneloop_eul_bk":
        Pk_terms["Pd1d1"] = (bv1["b1E"][:,None] * bv2["b1E"][:,None]
            * Pk_basis_funcs["Pnl"])
        Pk_terms["Pd1d2"] = (0.5 * (bv1["b1E"][:,None]*bv2["b2E"][:,None]
            + bv2["b1E"][:,None]*bv1["b2E"][:,None]) * Pk_basis_funcs["Pd1d2"])

        Pk_terms["Pd2d2"] = 0.25 * bv1["b2E"][:,None] * bv2["b2E"][:,None] * Pk_basis_funcs["Pd2d2"]

        Pk_terms["Pd1s2"] = (0.5 * (bv1["b1E"][:,None]*bv2["bsE"][:,None] +
            bv2["b1E"][:,None]*bv1["bsE"][:,None]) * Pk_basis_funcs["Pd1s2"])

        Pk_terms["Pd2s2"] = (0.25 * (bv2["b2E"][:,None]*bv1["bsE"][:,None] +
            bv1["b2E"][:,None]*bv2["bsE"][:,None]) * Pk_basis_funcs["Pd2s2"])

        Pk_terms["Ps2s2"] = 0.25 * bv1["bsE"][:,None]*bv2["bsE"][:,None] * Pk_basis_funcs["Ps2s2"]

        Pk_terms["sig3nl"] = (0.5*(bv1["b1E"][:,None]*bv2["b3nlE"][:,None] +
            bv2["b1E"][:,None]*bv1["b3nlE"][:,None]) * Pk_basis_funcs["sig3nl"])

        Pk_terms["k2P"] = ((bv1["b1E"][:,None]*bv2["bkE"][:,None]
            + bv2["b1E"][:,None]*bv1["bkE"][:,None]) * Pk_basis_funcs["k2P"])
...
def get_PXm_bias_red_dep(bias_values, Pk_basis_funcs, pt_type):
    """
    Get P_XX(k) - the tracer-matter power spectrum, generated by summing
    the P(k) terms in Pk_basis_funcs with the bias coefficients in
    bias_values. Also return the individual
    ...
    if pt_type in ['oneloop_eul_bk']:
        b1E, b2E, bsE, b3nl, bk = (bias_values["b1E"][:,None], bias_values["b2E"][:,None],
            bias_values["bsE"][:,None], bias_values["b3nlE"][:,None], bias_values["bkE"][:,None])
