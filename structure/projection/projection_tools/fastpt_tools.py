...
def get_bias_params_redshift(block, pt_type, sample):
    ...
    for param_name, default, order in zip(param_names, param_defaults, bias_order):
        ...
        bias_values[order] = np.poly1d(list(reversed(bias_coeff)))(zrange)
...
def get_PXX_bias_redshift_dependent(b_arrays, Pk_basis_funcs, pt_type):
    """
    Get P_XX(k) - the tracer-tracer power spectrum, generated by summing
    the P(k) terms in Pk_basis_funcs with the bias coefficients in
    bias_values_bin1 and bias_values_bin2. Also return the individual
    ...
    #use shorter variable names here as we'll be using
    #these dictionaries a lot!
    b1E, b2E, bsE, b3nl, bk = (b_arrays["b1E"], b_arrays["b2E"],
            b_arrays["bsE"], b_arrays["b3nlE"], b_arrays["bkE"])

    Pk_terms = {}

    if pt_type == "oneloop_eul_bk":
        Pk_terms["Pd1d1"] = (b1E**2
            * Pk_basis_funcs["Pnl"])
        Pk_terms["Pd1d2"] = (b1E*b2E * Pk_basis_funcs["Pd1d2"])

        Pk_terms["Pd2d2"] = 0.25 * b2E**2 * Pk_basis_funcs["Pd2d2"]

        Pk_terms["Pd1s2"] = (b1E*bsE * Pk_basis_funcs["Pd1s2"])

        Pk_terms["Pd2s2"] = (0.5 * b2E*bsE * Pk_basis_funcs["Pd2s2"])

        Pk_terms["Ps2s2"] = 0.25 * bsE**2 * Pk_basis_funcs["Ps2s2"]

        Pk_terms["sig3nl"] = (b1E*b3nl * Pk_basis_funcs["sig3nl"])

        Pk_terms["k2P"] = 2 * b1E*bk * Pk_basis_funcs["k2P"]
...
def get_PXm_bias_redshift_dependent(b_arrays, Pk_basis_funcs, pt_type):
    """
    Get P_XX(k) - the tracer-matter power spectrum, generated by summing
    the P(k) terms in Pk_basis_funcs with the bias coefficients in
    bias_values. Also return the individual
